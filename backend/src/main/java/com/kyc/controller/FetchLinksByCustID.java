            ...
            try {
                ...
                try (var mongoClient = MongoClients.create(mongoUriString)) {
                    ...
                    for(String dType : docType) {
                        ...
                        switch(dType) {
                            case "Aadhaar Card":
                                Document resultAadhaar = aadhaar.find(new Document("cust_id", custId)).projection(projection).first();
                                map.put("Aadhaar Card", resultAadhaar);
                                break;
                            case "PAN Card":
                                Document resultPan = pan.find(new Document("cust_id", custId)).projection(projection).first();
                                map.put("PAN Card", resultPan);
                                break;
                            case "Credit Card":
                                Document resultCreditCard = creditcard.find(new Document("cust_id", custId)).projection(projection).first();
                                map.put("Credit Card", resultCreditCard);
                                break;
                            case "Cheque":
                                Document resultCheque = cheque.find(new Document("cust_id", custId)).projection(projection).first();
